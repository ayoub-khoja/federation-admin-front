...
interface AddDesignationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (designationData: DesignationData) => void;
}
...
export default function AddDesignationModal({ isOpen, onClose, onSubmit }: AddDesignationModalProps) {
  const [formData, setFormData] = useState<DesignationData>({
    date: '',
    stade: '',
    equipe1: '',
    equipe2: '',
  ...
  const [errors, setErrors] = useState<Partial<DesignationData>>({});
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (errors[name as keyof DesignationData]) {
      setErrors(prev => ({
        ...prev,
        [name]: undefined
      }));
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<DesignationData> = {};
      ...
      <div className="bg-white/10 backdrop-blur-md rounded-3xl shadow-2xl border border-white/30 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 border-b border-white/20">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-white">➕ Nouvelle Désignation</h2>
            <button
              onClick={handleClose}
          ...
          <p className="text-white/70 mt-2">Ajouter une nouvelle désignation de match</p>
        </div>
